...
(function (twitterlib, window) {
  var guid = +new Date,
      document = window.document || { getElementsByTagName: function () { return [] }, getElementById: function () { return {} } },
  ...
  function render(tweet) {
    ...
    html += window[twitterlib].ify.clean(tweet.text);
    ...
    html += tweet.created_at + '">' + window[twitterlib].time.datetime(tweet.created_at) + '</span></a>';
  ...
  function load(url, options, callback) {
      ...
      return function (tweets) {
        ...
        if (tweets.results) {
          ...
          while (i--) {
            ...
            tweets[i].source = window[twitterlib].ify.entities(tweets[i].source);
        ...
        callback.call(window[twitterlib], tweets, options);
        // clean up
        clean(guid);
      };
    })(guid, options);
    ...
    if (!caching || options.page <= 1 || (caching && sessionStorage.getItem(twitterlib + '.page' + options.page) == null)) {
      ...
      callback.call(window[twitterlib], JSON.parse(sessionStorage.getItem(twitterlib + '.page' + options.page + '.tweets')), options);
    } 
  }
  ...
  window[twitterlib].custom('search');
  window[twitterlib].custom('timeline');
  window[twitterlib].custom('favs');
})('twitterlib', this);